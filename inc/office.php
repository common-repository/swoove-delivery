...
class SwooveBO
{
	public $deliveryURL = 'https://live.swooveapi.com/delivery/create-delivery?platform=swoove_woocommerce_plugin&app_key=';
	...
	public function __construct()
	{
		...
		add_action('admin_enqueue_scripts', [$this, 'load_adm_scripts']);
	}

	function load_adm_scripts($hook)
	{
		wp_enqueue_script('swoove-js', plugins_url('../assets/js/swoove.js', __FILE__), ['jquery']);
	}
	...
	public function swoove_status($column)
	{
		...
		if ($column == 's_order-status')
			echo esc_html("<mark class='order-status'><span>" . get_post_meta($the_order->id, 'swoove_delivery_status', true) . "</span></mark>");
	}
	...
	function swoove_dispatch($order)
	{
		...
		if ($_GET['sv_action'] == 'delivery' && $order->id == $_GET['thisOrder']) {
			...
			if ($isDebug == 'yes')	$this->$deliveryURL = 'https://test.swooveapi.com/delivery/create-delivery?platform=swoove_woocommerce_plugin&app_key=';
...
?>
		<div class="order_data_column" style="width: 100%">
			...
			<div class="address">
				...
				echo esc_html('<p><b>' . __('Delivery Code', 'swoove') . ': </b>' . (empty($sd_code) ? 'Not Created' : $sd_code) . '</p>');
				echo esc_html('<p><b>' . __('Secret Code', 'swoove') . ': </b>' . (empty($ss_code) ? 'Not Created' : $ss_code) . '</p>');
				echo esc_html('<p><b>' . __('Estimate', 'swoove') . ': </b>' . $es_id . '</p>');
				echo esc_html('<p><b>' . __('Status', 'swoove') . ': </b>' . $status . '</p>');
				echo esc_html('<p><b>' . __('DropOff', 'swoove') . ': </b>' . $address . '</p>');
				if (!empty($track))
					echo esc_html('<a href="' . $track . '"  target="_blank" class="button"> Track Delivery
